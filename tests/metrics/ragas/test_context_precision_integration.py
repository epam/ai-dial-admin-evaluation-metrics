"""Integration tests for ragas.context_precision with recorded LLM responses.
...
@pytest.mark.vcr
def test_context_precision_evaluate_via_api(app_client):
    request = {
    ...
    data = response.json()
    assert data["metric_name"] == "ragas.context_precision"
    output = ContextPrecisionMetric.Output.model_validate(data["output"])
