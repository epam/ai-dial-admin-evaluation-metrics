"""Integration tests for deepeval.contextual_precision with recorded LLM responses.
...
@pytest.mark.vcr
def test_contextual_precision_evaluate_via_api(app_client):
    request = {
        "metric_name": "deepeval.contextual_precision",
    ...
    assert data["metric_name"] == "deepeval.contextual_precision"
    output = ContextualPrecisionMetric.Output.model_validate(data["output"])
